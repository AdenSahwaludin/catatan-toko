      ...
      <div class="chart-stats mb-4">
        <v-row dense>
          ...
          <v-col cols="6" sm="3">
            <div class="stat-item">
              <div class="stat-value">
                {{ formatCurrency(periodStats.averageTransaction) }}
              </div>
...
const createChart = async () => {
  ...
  if (!isMounted.value || !chartCanvas.value) {
    console.warn(
      "Chart creation skipped - component not mounted or canvas unavailable"
    );
    return;
  }
  ...
  try {
    ...
    if (!isMounted.value || !chartCanvas.value) {
      console.warn(
        "Chart creation aborted - component state changed after delay"
      );
      return;
    }
